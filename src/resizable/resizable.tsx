...
export function HorizontalResizableDoublePane(
    ...
    resizerPercentage = Math.min(Math.max(resizerPercentage, 0), 100);
    ...
    useListenerOnWindow({
        ...
        listener: ({ changedTouches: [{ clientX }] }) => dragResizer(clientX)
    });
    useListenerOnWindow({
...
export function VerticalResizableDoublePane(
    ...
    useListenerOnWindow({
        listenerType: "touchmove",
        listener: ({ changedTouches: [{ clientY }] }) => dragResizer(clientY)
    });
    useListenerOnWindow({
        listenerType: ["mouseup", "blur", "touchend"],
        listener: setResizerActive.bind(null, false)
